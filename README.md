...
``` r
pickScore$pickValue <-predict(scam(pickScore$meanPickVORP~s(pickScore$numberPickOverall,
                                                            k=5,bs="mdcx")),se=FALSE)
...
picks is 125.6. There are rumors that they are interested in trading
those two picks for the second overall pick, which is worth 92.1 points.
...
sum of those two picks is about equivalent to the 12th pick.
